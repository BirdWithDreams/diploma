    ...
    for ref_wav, df_group in tqdm(df_speaker):
        ...
        for _, row in df_group.iterrows():
            ...
            if not os.path.isfile(tts_wav_path):
                ...
                if missing_files > 3:
                    raise RuntimeError(
                        f"More than 3 wave files are missing for the CSV {args.csv_path}!! Please check it, because It can compromise the evaluation.")
                continue

            meta_dict, meta_dict_full = compute_metrics(tts_wav_path, ref_wav, row["text"],
                                                        ref_speaker_embedding=speaker_embedding,
                                                        language=row["language"], ref_dBFS=ref_dBFS)
